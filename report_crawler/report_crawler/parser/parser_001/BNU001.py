...
def Parser(text, sub_linefeed):
	...
	# time
	time_pattern = re.compile(u"报(?:.*?){0,1}告(?:.*?){0,1}时(?:.*?){0,1}间[：:. ](.*?)\n", re.S)
	...
	# abstract
	abstract_pattern = re.compile(u"(?:摘要|内容|Bio)[：:.]([\s\S]*)(?:(?:报告|主讲)人|Abstract)(?:.*?){0,1}[：:.]", re.S)
	...
	if len(messages['abstract']) == 1:
		...
		abstract_pattern = re.compile(u"(?:摘要|内容|Bio)[：:.]([\s\S]*)", re.S)
		messages['abstract'] = re.findall(abstract_pattern, text)
		if len(messages['abstract']) == 1:
			messages['abstract'] = sub_linefeed(messages['abstract'][0].strip())
		else:
			messages['abstract'] = ''
	if u"附件下载" in messages['abstract']:
		messages['abstract'] = re.findall(u"([\s\S]*)附件下载", messages['abstract'], re.S)[0]

	# biography
	biography_pattern = re.compile(u"人(?:简介|介绍|Abstract)[：:.]([\s\S]*)(?:(?:报告|讲座)(?:摘要|内容|简介)|Bio)[：:.]", re.S)
	...
	if len(messages['biography']) == 1:
		...
		if len(messages['biography']) == 1:
			...
			messages['biography'] = ''
	if u"附件下载" in messages['biography']:
		messages['biography'] = re.findall(u"([\s\S]*)附件下载", messages['biography'], re.S)[0]
